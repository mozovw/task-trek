...
- 如果你想通过 useMessage 使用信息，你需要把调用其方法的组件放在 n-message-provider 内部并且使用 useMessage 去获取 API。
...
#### 选择 1
使用 createDiscreteApi。如果你想使用它，请认真阅读它的注意事项。你最好不要把它和 useMessage 在同一 App 中混用。
